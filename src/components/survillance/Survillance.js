...
const FeatureCard = ({ imgSrc, imgAlt, description, imgWidth }) => (
  <div className="flex gap-1 pl-4 pr-7 py-6 mt-7 rounded-xl border border-sky-700 border-solid max-md:px-5">
    <img loading="lazy" src={imgSrc} alt={imgAlt} className="shrink-0 aspect-1.5 w-[40%]" style={{ width: imgWidth || 'inherit' }} />
    <div className="pl-5">{description}</div>
  </div>
);
    ...
    <div className="flex overflow-hidden relative flex-col justify-center items-center px-16 min-h-[783px] max-md:px-5">
      ...
      <div className="flex relative justify-center items-center px-16 py-20 w-full max-w-[1440px] max-md:px-5 max-md:max-w-full">
        <div className="flex flex-col items-center mt-3.5 w-full max-w-[1234px] max-md:max-w-full">
          <h1 className="text-5xl font-bold text-white text-center bg-clip-text leading-[68.88px] max-md:max-w-full">Powerful long range outdoor surveillance</h1>
          <p className="mt-4 text-3xl font-medium tet white text-center leading-[68.88px] text-neutral-400 max-md:max-w-full">Dual Antenna for Long Range Transmission</p>
                ...
                <div className="flex flex-col text-white">
                  ...
                  <div className="flex gap-1 px-14 py-6 mt-7 rounded-xl border border-sky-700 border-solid max-md:px-5">
                    {featuresLeft.slice(2).map((feature, index) => (
                      <img key={index} loading="lazy" src={feature.imgSrc} alt={feature.imgAlt} className="shrink-0 self-start " />
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="flex gap-4 mt-20 text-3xl font-semibold tracking-wider text-center capitalize leading-[68px] max-md:flex-wrap max-md:mt-10">
            <img src="./images/powerfull-wifi-icon.png" alt="Wi-Fi & Ethernet Icon" className="shrink-0 aspect-[1.27]" />
